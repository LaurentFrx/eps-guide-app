...
const COMMENT_RE =
  /\b(Probablement|Peut[-\u2010-\u2015]?être|Peut[-\u2010-\u2015]?etre)\b/i;
...
const checkValue = (scope: string, field: string, value: string) => {
  ...
  if (!COMMENT_RE.test(value)) return;
...
if (!issues.length) {
  console.log("No editor comments found.");
  process.exit(0);
}

console.error(`Editor comment check failed (${issues.length}).`);
